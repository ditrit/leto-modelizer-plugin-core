...
class DefaultDrawer {
  ...
  initActions() {
    ...
    this.actions.dragComponent = new DragComponentAction(this.pluginData, this.viewport, this.layout);
    ...
    this.actions.resizeComponent = new ResizeComponentAction(this.pluginData, this.viewport, this.layout);
    this.actions.toggleSelection = new ToggleSelectionAction(this.pluginData, this.viewport, this.layout);
    this.actions.selectionAll = new SelectionAllAction(this.pluginData, this.viewport, this.layout);
    this.actions.deselectionAll = new DeselectionAllAction(this.pluginData, this.viewport, this.layout);
    ...
    if (!this.readOnly) {
      this.d3.select('body')
        .on('keydown', (event) => {
          ...
          if (this.pluginData.keysBinding.selection.includes(event.key)) {
            this.states.selection = true;
          }
          if (this.pluginData.keysBinding.deleteObject.includes(event.key)) {
            haveToDraw = this.actions.deleteAction.execute(event);
          }
          if (this.pluginData.keysBinding.selectAll.includes(event.key)) {
            haveToDraw = this.actions.selectionAll.execute(event);
          }
          if (this.pluginData.keysBinding.deselectAll.includes(event.key)) {
